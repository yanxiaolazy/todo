    ...
    <div className={`${prefix}`}>
      {
        lists?.map(list => {
          ...
          return(
            <div className={`${prefix}-container`} key={list.uid}>
              <div className={`${prefix}-checked`}>
                <DisplayType dataId={params.file} className={`${prefix}-checked-item`} file={list.name}/>
              </div>
              ...
              <span className={`${prefix}-delete`} data-id={params.file} onClick={onDelete}>删除</span>
            </div>
          );
        })
      }
